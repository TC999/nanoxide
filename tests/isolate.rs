...
fn edit_redraw_alone() {
    ...
    nanoxide::winio::edit_redraw(&cur, UpdateType::Flowing);
}
...
fn offscreen_checks_alone() {
    setup();
    nanoxide::text::inject(b"abc", 3);
    let above = nanoxide::winio::current_is_above_screen();
    let below = nanoxide::winio::current_is_below_screen();
    let off = nanoxide::winio::current_is_offscreen();
...
fn adjust_viewport_alone() {
    setup();
    nanoxide::text::inject(b"abc", 3);
    nanoxide::winio::adjust_viewport(UpdateType::Flowing);
}
...
fn handle_right_key_alone() {
    ...
    let handled = nanoxide::winio::handle_input_key(nanoxide::definitions::KEY_RIGHT);
    assert!(handled);
    let x = with_global(|g| g.openfile.as_ref().unwrap().borrow().current_x);
    assert_eq!(x, 1);
}
...
fn empty_buffer_startup_no_panic() {
    ...
    nanoxide::winio::place_the_cursor();
}
...
fn nonexistent_file_startup_no_panic() {
    setup();
    nanoxide::files::open_buffer("definitely_missing_file_xyz.txt");
...
fn write_it_out_saves_file() {
    ...
    let path = std::env::temp_dir().join(format!("nanoxide_test_{}.txt", std::process::id()));
...
fn write_file_promptbar_displays() {
    ...
    nanoxide::prompt::draw_the_promptbar();
    assert_eq!(nanoxide::prompt::get_answer(), "test_wo.txt");
...
fn absorb_character_adds_to_answer() {
    ...
    // 输入 'a'（无快捷键）
    nanoxide::prompt::absorb_character(97, None);
    assert_eq!(nanoxide::prompt::get_answer(), "a", "普通字符应被吸收进 answer");
    // 再输入 'b'
    nanoxide::prompt::absorb_character(98, None);
    assert_eq!(nanoxide::prompt::get_answer(), "ab");
}
...
fn absorb_enter_does_not_block() {
    ...
    nanoxide::prompt::absorb_character(13, Some(FunctionId::DoEnter));
    assert_eq!(nanoxide::prompt::get_answer(), "test.txt");
}
...
fn statusbar_backspace_and_delete_remove_chars() {
    ...
    nanoxide::prompt::handle_editing(FunctionId::DoBackspace);
    assert_eq!(nanoxide::prompt::get_answer(), "abc.tx", "退格应删除光标前字符");

    with_global_mut(|g| {
        g.answer = Some("abc.txt".to_string());
        g.typing_x = 1;
    });
    nanoxide::prompt::handle_editing(FunctionId::DoDelete);
    assert_eq!(nanoxide::prompt::get_answer(), "ac.txt", "Delete 应删除光标处字符");
}
...
fn linenumbers_margin() {
    setup();
    let m0 = nanoxide::winio::current_margin();
    assert_eq!(m0, 0, "默认不显示行号");
    SET(LINE_NUMBERS);
    let m1 = nanoxide::winio::current_margin();
    assert_eq!(m1, 2, "单行文件行号位数1 + 空格 = 2");
    /* 构造 10 行：9 次回车 + 每行一个字符。 */
    for _ in 0..9 {
        nanoxide::text::inject(b"a", 1);
        nanoxide::text::do_enter();
    }
    nanoxide::text::inject(b"a", 1);
    nanoxide::files::prepare_for_display();
    let m2 = nanoxide::winio::current_margin();
    assert_eq!(m2, 3, "10 行文件行号位数2 + 空格 = 3");

    UNSET(LINE_NUMBERS);
    assert_eq!(nanoxide::winio::current_margin(), 0, "关闭后恢复 0");
}
...
fn args_after_filename_parsed() {
    setup();
    // 文件名在前，选项在后
    let f = nanoxide::global::parse_args(&[
        "nax".to_string(), "file.txt".to_string(), "-l".to_string(),
    ]);
    assert_eq!(f.as_deref(), Some("file.txt"));
    assert!(ISSET(LINE_NUMBERS), "-l 在文件名后应生效");
    UNSET(LINE_NUMBERS);
    // 选项在前，文件名在后（常规形式）
    let f2 = nanoxide::global::parse_args(&[
        "nax".to_string(), "--linenumbers".to_string(), "file.txt".to_string(),
    ]);
    assert_eq!(f2.as_deref(), Some("file.txt"));
    assert!(ISSET(LINE_NUMBERS));
    UNSET(LINE_NUMBERS);
    // "--" 之后不再解析选项
    let f3 = nanoxide::global::parse_args(&[
        "nax".to_string(), "--".to_string(), "file.txt".to_string(), "-l".to_string(),
    ]);
...
fn help_init_and_wrap_works() {
    ...
    // 换行入新缓冲（若死循环会卡住该测试）
    nanoxide::help::wrap_help_text_into_buffer();
